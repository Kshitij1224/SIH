          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-4">
              {appointments.map((appointment, index) => (
                <div 
                  key={index} 
                  className="group flex items-center justify-between p-4 bg-gray-50 rounded-lg transition-all duration-200 hover:shadow-md hover:border-l-4 hover:border-blue-500 hover:pl-3.5"
                >
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 hover:shadow-md hover:scale-105 transform transition-all duration-200 text-sm font-medium flex items-center gap-1.5 group/button"
                  >
                    <span>Join Call</span>
                    <svg className="w-3.5 h-3.5 group-hover/button:translate-x-0.5 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-3">
              {medications.map((med, index) => (
                <div 
                  key={index} 
                  className="group flex items-center justify-between p-3 bg-gray-50 rounded-lg transition-all duration-200 hover:bg-white hover:shadow-sm hover:border hover:border-gray-100 cursor-pointer"
                >
                  <div className="transition-transform duration-200 group-hover:translate-x-1">
                  ...
                  <div 
                    className={`w-4 h-4 rounded-full transition-all duration-300 group-hover:scale-125 ${
                      med.name === 'Vitamin D3' || med.name === 'Ibuprofen' 
                  ...
                  >
                    <div className="relative w-full h-full flex items-center justify-center">
                      {med.name === 'Vitamin D3' || med.name === 'Ibuprofen' ? (
                        <span className="absolute text-white text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity">
                          !
                        </span>
                      ) : (
                        <span className="absolute text-white text-[10px] font-bold opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {vitals.gauges.map((g) => {
              ...
              return (
                <div key={g.key} className="group relative overflow-hidden rounded-xl border border-gray-100 bg-gradient-to-br from-white to-gray-50 p-4 transition-all duration-300 hover:shadow-md hover:border-gray-200 hover:-translate-y-0.5">
          ...
          <div className="group mt-6 overflow-hidden rounded-xl border border-gray-100 bg-white transition-all duration-300 hover:shadow-md hover:border-blue-100">
            <div className="px-4 pt-4 transition-all duration-300 group-hover:bg-gray-50/50">
              <div className="flex items-center justify-between mb-2 group-has-[:hover]:text-blue-600 transition-colors">
                <div className="flex items-center gap-2">
                  <div className="p-2 rounded-md bg-violet-100">
                    <Activity className="w-4 h-4 text-violet-600" />
                  </div>
                  <div className="text-sm font-medium text-gray-900 group-has-[:hover]:text-blue-600 transition-colors">
                    Blood Pressure
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-end gap-2 transition-all duration-300 group-has-[:hover]:translate-x-1">
                  <div className="text-2xl font-bold text-gray-900 group-has-[:hover]:text-blue-700 transition-colors">
                    <span className="relative">
                      <span className="relative z-10">{vitals.bloodPressure.systolic}</span>
                      <span className="absolute -bottom-1 left-0 right-0 h-1 bg-blue-100 -z-0 opacity-0 group-has-[:hover]:opacity-100 transition-opacity"></span>
                    </span>
                    /
                    <span className="relative">
                      <span className="relative z-10">{vitals.bloodPressure.diastolic}</span>
                      <span className="absolute -bottom-1 left-0 right-0 h-1 bg-blue-100 -z-0 opacity-0 group-has-[:hover]:opacity-100 transition-opacity"></span>
                    </span>
                  </div>
                  <div className="text-sm text-gray-500 mb-0.5 group-has-[:hover]:text-gray-700 transition-colors">
              ...
              {(() => {
                ...
                return (
                  <div className="mt-3 grid grid-cols-3 gap-2 text-center group-has-[:hover]:opacity-100 opacity-90 transition-opacity">
            ...
            <div className="mt-3 px-4 pb-4 transition-all duration-300 group-has-[:hover]:bg-gray-50/30 rounded-b-xl">
              <div className="h-20 relative group/graph">
                  ...
                  {(() => {
                    ...
                    return maps.map((mv, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="relative flex flex-col items-center justify-end h-full cursor-pointer focus:outline-none group/bar"
                        ...
                        >
                          ...
                          <div 
                            className="w-full rounded-t-md transition-all duration-300 group-hover/bar:opacity-100 group-hover/bar:rounded-t-lg" 
                            style={{ 
                              height: `${h}%`, 
                ...
                <div key={group}>
                  ...
                  <ul className="space-y-3">
                    {allMeds.filter(m => m.status === group).map((m, idx) => {
                      const bgColor = group === 'present' 
                        ? 'from-green-50 to-white' 
                        : group === 'intermittent' 
                          ? 'from-amber-50 to-white' 
                          : 'from-red-50 to-white';
                      const borderColor = group === 'present' 
                        ? 'border-green-100 hover:border-green-200' 
                        : group === 'intermittent' 
                          ? 'border-amber-100 hover:border-amber-200' 
                          : 'border-red-100 hover:border-red-200';
                      const shadowColor = group === 'present' 
                      ...
                      return (
                        <motion.li 
                          key={idx} 
                          className={`p-4 rounded-xl border ${borderColor} bg-gradient-to-br ${bgColor} transition-all duration-300 hover:shadow-lg ${shadowColor} hover:-translate-y-0.5 cursor-pointer`}
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`w-2 h-2 rounded-full ${group==='present' ? 'bg-green-500' : group==='intermittent' ? 'bg-amber-500' : 'bg-red-500'}`}></span>
                              </div>
                              <div className="mt-1 text-sm text-gray-600">
                                <span className="font-medium">{m.dosage}</span> • {m.frequency}
                              </div>
                              {m.ended && (
